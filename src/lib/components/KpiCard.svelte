<script lang="ts">
	import { euro, euroSimple } from '$lib/features/dashboard';
	...
	let { totalIncome, currentPrsDonation }: Props = $props();
</script>
...
<div class="grid grid-cols-2 gap-2.5 sm:gap-3">
	...
	>
		<p class="mb-1 text-[0.58rem] font-bold tracking-widest text-emerald-600 uppercase">
			{DASHBOARD_LANG.kpi.operational.label.original}
		</p>
		<p
		...
		<p class="mt-1 text-[0.58rem] font-medium text-emerald-700/60">
			{DASHBOARD_LANG.kpi.operational.period.original} &middot;
			{DASHBOARD_LANG.kpi.operational.period.sub}
		</p>
	</div>
	<div
		class="rounded-2xl border border-green-100 bg-white/80 px-3.5 py-3 shadow-sm backdrop-blur-sm sm:px-4 sm:py-3.5"
	>
		<p class="mb-1 text-[0.58rem] font-bold tracking-widest text-green-600 uppercase">
			{DASHBOARD_LANG.kpi.prs.label.original}
		</p>
		<p
		...
		<p class="mt-1 text-[0.58rem] font-medium text-green-700/60">
			{DASHBOARD_LANG.kpi.prs.period.original} &middot; {DASHBOARD_LANG.kpi.prs.period.sub}
		</p>
	</div>
</div>
